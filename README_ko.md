...
- **의도 보정**: LLM 기반 유사어 확장(옵션)으로 검색 의도를 더 정확히 반영합니다.
...
![시스템 파이프라인](figs/pipeline_ai.png)
...
3. 검색 의도가 충분히 반영되지 않을 경우, 유사어 확장을 켜서 후보 폭을 조정합니다.
...
3. 텍스트는 유사어 확장(옵션)을 거쳐 임베딩을 만들고, pgvector 후보와 OpenSearch BM25 후보를 합쳐 MetaCLIP 유사도로 재정렬합니다.
...
### UI 상호작용
1. 검색 결과는 카드와 페이지네이션으로 제공되며, 재검색 시 즉시 갱신됩니다.
...
- 기본 가중치는 **DINO:MetaCLIP = 0.5:0.5**로 고정됩니다.
- 텍스트 검색은 **MetaCLIP2 임베딩 + OpenSearch BM25**를 함께 사용합니다. BM25는 후보 확장용이며 최종 정렬은 MetaCLIP 유사도로 결정됩니다. 유사어 확장은 후보 범위를 넓힐 때 사용합니다.
...
- 이미지/텍스트 결과를 동시에 비교하고, 유사어 확장 토글로 후보 범위를 조정할 수 있습니다.
