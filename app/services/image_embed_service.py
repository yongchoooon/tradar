...
class ImageEmbedder:
    ...
    def _ensure_backend(self):
        if self._backend is None:
            self._backend = get_image_backend(self._backend_name)
        return self._backend

    def encode(self, image_bytes: bytes) -> Dict[str, List[float]]:
        backend = self._ensure_backend()
        return backend.encode(image_bytes)

    def encode_batch(self, images: Iterable[bytes]) -> List[Dict[str, List[float]]]:
        backend = self._ensure_backend()
        if hasattr(backend, "encode_batch"):
            return backend.encode_batch(list(images))
        return [backend.encode(image) for image in images]
