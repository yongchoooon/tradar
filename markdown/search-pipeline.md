...
5. **Top-K 선정**
   - 상위 K개 후보를 단일 리스트로 반환합니다. 기본값은 20이지만 프런트엔드는 `k=200`으로 요청해 이후 클라이언트 단에서 페이지를 나눕니다.
...
3. **쿼리 벡터 생성**
   - 원본 상표명은 가중치 4.5, 유사어는 0.5로 가중합하여 MetaCLIP2 텍스트 임베딩을 만듭니다.
...
7. **Top-K 선정**
   - MetaCLIP 유사도와 프롬프트 필터를 반영해 Top-K를 선택합니다. 기본값은 20이며, 클라이언트는 최대 200개를 받아 그리드에서 페이지 단위로 노출합니다. 선택한 상품 분류 기반의 인접/비인접 구분은 향후 버전에서 추가됩니다.
...
- `image_top`: 이미지 Top-K 후보 리스트 (기본 20, 클라이언트는 최대 200개를 요청해 페이징)
- `image_misc`: Top-K 이외의 후보 중 상태가 등록/공고가 아닌 항목 (최대 10개, 등록/공고만 정상 상태로 간주)
- `text_top`: 텍스트 Top-K 후보 리스트 (기본 20, 클라이언트는 최대 200개를 요청해 페이징)
