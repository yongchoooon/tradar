...
- 구성: **FastAPI 백엔드 + Vite/React 프런트엔드 + PostgreSQL/pgvector + OpenSearch + LangGraph**

## 2) 레포 구조 (핵심 경로)
- `app/`: FastAPI 백엔드 핵심 코드
  - `api/`: 라우터 (`/search/multimodal`, `/goods/search`, `/media`, `/simulation/*`)
...
## 3) 백엔드 아키텍처
### 3-1. 엔드포인트
- `POST /search/multimodal`: 이미지/텍스트 멀티모달 검색
- `GET /goods/search`: 상품/서비스류 검색
- `GET /media`: 로컬 이미지 파일 프록시 제공
...
### 3-2. 환경 변수/환경 제약
- `APP_ENV=prod`이면 `DATABASE_URL`, `OPENSEARCH_URL`, `OPENAI_API_KEY`, `KIPRIS_ACCESS_KEY` 필수
- `CORS_ALLOWED_ORIGINS`는 운영 환경에서 필수
- 개발 환경에서는 기본값을 세팅하도록 처리

## 4) 검색 파이프라인 상세
### 4-1. 입력 스키마 (`SearchRequest`)
- `image_b64`(필수), `text`(선택)
...
- **가중치**: 원문 4.5, 유사어 0.5
- **BM25 확장**: OpenSearch Top-N 후보 추가
...
- 카드에 상태 배지(등록/공고/거절 등), 유사도, 썸네일 표시
...
- **운영**: ECS Fargate 태스크 정의 + SSM Parameter Store 기반 비밀값 주입
- 운영 구성은 RDS/OpenSearch/S3/CloudFront 전제를 둔 문서/설정이 존재
