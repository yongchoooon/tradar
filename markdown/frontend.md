...
- 최초 검색 시 `k=200`으로 요청해 충분한 후보를 받아오고, 이후 재검색은 서버 응답의 `query.k` 값을 사용합니다.
...
### GoodsSearchPanel
- 키워드로 유사군을 찾는 보조 UI
- 체크된 유사군은 `selectedGroups`에 저장되어 검색 요청에 포함됩니다. 현재 순위 계산에는 사용되지 않지만, 검색/시뮬레이션 실행을 위해 최소 한 개 이상 선택해야 하며 선택 목록이 그대로 프롬프트 컨텍스트에 주입됩니다.
...
- `시뮬레이션 실행`을 누르면 상태가 `collecting`으로 전환되고 SSE(`/simulation/stream/{job_id}`)로 진행 상황을 수신합니다.
- 상태 카드는 `collecting` → `loading`(서버에서 `simulating`으로 보내는 값을 클라이언트에서 매핑) → `complete` 순으로 굵기·색상이 바뀌며, 각 단계마다 "시뮬레이션을 준비하고 있습니다"/"진행 중입니다"/"내용을 요약하는 중입니다" 처럼 한 줄씩 추가됩니다.
...
- 각 후보별 카드는 접기/펼치기 형태로 제공되며, 펼쳤을 때 LLM(채점자 에이전트)이 산출한 점수·근거, 리포터 Markdown, 참고 요소를 확인할 수 있습니다.
...
3. `ResultSection`은 이미지/텍스트 최대 200건을 받아 18개씩 페이지네이션하고, "기타"(등록/공고 외) 섹션도 함께 렌더링합니다.
4. 오류 발생 시 `error` 메시지를 상단에 노출합니다.

## 스타일 가이드
- 기본 스타일은 Vite 템플릿 + `frontend/src/index.css`
- `:root`에 정의된 `--viewport-scale`, `--space-scale`, `--fluid-gap` 변수가 전체 폰트/패딩/갭을 뷰포트 폭에 맞춰 자동 조절합니다. 큰 모니터에서는 100% 크기를 유지하고, 14~16인치 화면에서는 약 80% 수준으로 축소되도록 설계했으므로 컴포넌트 크기를 수정할 때 해당 변수를 우선 확인하세요.
- 검색 폼, 드롭존, 결과 카드 모두 동일한 박스 그림자를 사용해 레이아웃 일관성 유지
- 반응형: 768px 이하에서는 결과 카드를 한 줄에 하나만 표시하고, 중간 폭(1200~1500px)에서는 검색/시뮬레이션 컬럼 비율이 70:30으로 자동 재배치됩니다.
